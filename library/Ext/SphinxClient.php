...
/**
  sphinx index SQL condition:
  SELECT goods_id,  goods_sn,  goods_name  \
  FROM efoxshop_de_goods where is_on_sale=1
 * require('sphinxapi.php');
  $s = new SphinxClient();
  $s->SetServer('172.16.1.24', 9312);
  //    $s->SetConnectTimeout(0.3);
  //    $s->SetMaxQueryTime(3);

  $keyword="Hamburger-Form";
  $res = $s->query($keyword, 'laugh'); //执行检索，关键字, 索引名 多个索引用,分割，查询全部索引用*表示

  //    $res=$s->SetFilter ( "cat_id", array ( 233 ) );
  echo "<PRE>";
  print_r($res);
 */
/////////////////////////////////////////////////////////////////////////////
