...
class Pub_Image
{
        ...
        public $images_cache_dir = 'temp/bmz_caches';
        ...
        public function upload_image($upload, $dir = '', $img_name = '')
        {
                ...
                if (empty($dir))
                {
                        /* 创建当月目录 */
                        $dir = date('Ym');
                        $dir = APPLICATION_PATH . $this->images_dir . '/' . $dir . '/';
                } else
                {
                        /* 创建目录 */
                        $dir = APPLICATION_PATH . $this->data_dir . '/' . $dir . '/';
                        if ($img_name)
                        {
                ...
                {
                        return str_replace(APPLICATION_PATH, '', $img_name);
                } else
                {
        ...
        public function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor = '')
        {
                ...
                {
                        return str_replace(APPLICATION_PATH, '', str_replace('\\', '/', realpath($img)));
                }
                ...
                if (empty($path))
                {
                        $dir = APPLICATION_PATH . $this->images_dir . '/' . date('Ym') . '/';
                } else
                {
                        $dir = $path;
                }
                ...
                {
                        return str_replace(APPLICATION_PATH, '', $dir) . $filename;
                } else
                {
        ...
        public function add_watermark($filename, $target_file = '', $watermark = '', $watermark_place = '', $watermark_alpha = 0.65)
        {
                ...
                {
                        return str_replace(APPLICATION_PATH, '', str_replace('\\', '/', realpath($filename)));
                }
                ...
                if ($path)
                {
                        return str_replace(APPLICATION_PATH, '', str_replace('\\', '/', $path));
                } else
                {
                        $this->error_msg = $GLOBALS['_LANG']['writting_failure'];
                        $this->error_no = ERR_DIRECTORY_READONLY;
        ...
        public function make_thumb_aecmp($img, $thumb_width = 0, $thumb_height = 0)
        {
                $gd = $this->gd_version(); //获取 GD 版本。0 表示没有 GD 库，1 表示 GD 1.x，2 表示 GD 2.x
                if ($gd == 0)
                {
                ...
                $img_basename = substr(basename($img), 0, strrpos(basename($img), '.'));
                $dir = APPLICATION_PATH . $this->images_cache_dir . '/' . substr($img_basename, 0, 1) . '/';
                ...
                /* 如果文件名为空，生成不重名随机文件名 */
                $filename = $img_basename . '.image.' . $thumb_width . 'x' . $thumb_height;
