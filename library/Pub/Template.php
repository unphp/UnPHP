...
class Pub_Template
{
        ...
        public $_var = array();
        public $_echash = '554fcae493e564ee0dc75bdf2ebf94ca_xiaotangren';
        ...
        public function __construct($template_dir,$themes)
        {
                ...
                $this->direct_output = false;  // 不缓存，直接输出。。。
                // 所谓编译缓存，指将模板smarty标签，翻译成php和html混合的文件后，生成的缓存。
                $this->force_compile = true;   // 默认开启编译缓存
                $this->caching = true;   // 默认开启静态缓存
        }
         ...
         * @return type
         */
        static public function mode($template_dir='')
        {
                if (self::$smarty == null)
                {
                        if($template_dir===''){
                                $template_dir = Yaf_Registry::get('template_dir');
                        }
                        self::$smarty = new self($template_dir);
                }
                return self::$smarty;
        }
                ...
                else
                {
                        ...
                        else
                        {
                                ...
                                else
                                {
                                        ...
                                        if ($cache_id)
                                        {
                                                $cachefile = $this->getCachefilepath($filename_cp,$cache_id);
        ...
        {
                $compile_dir_file = str_replace($this->template_dir, $this->themes.'/', $filename);
...
//                                                if($s[1]=='')
        ...
        protected function smarty_prefilter_preCompile($source)
        {
        ...
        protected function getCachefilepath($filename_cp,$cache_id)
        {
                
                $filename_cp = str_replace(DS, '_', $filename_cp);
                $cachename = $this->themes . '_' . basename($filename_cp, strrchr($filename_cp, '.')) . '_' . $cache_id;
                $hash_dir = $this->cache_dir . substr(md5($cachename), 0, 2) . '/' . substr(md5($cachename), 3, 2);
